    ...
    <section style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 1240 }}>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 12, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div className="text-muted" style={{ fontSize: 12.5, marginTop: 4 }}>All figures are demo data — never real. Every OM is source-grounded and export-gated by the Three-Lens Review.</div>
        </div>
      ...
      {data && data.drafts.length === 0 && (
        <div style={{ border: "2px dashed var(--color-divider)", padding: "40px 28px", maxWidth: 640 }}>
          ...
          <p className="text-muted" style={{ fontSize: 13, margin: "0 0 14px" }}>Attach an authorized rent roll, financials, and comps, pick a brand kit and a template you own, and AgentOS assembles a source-linked draft you can verify and export.</p>
          ...
          {data.drafts.map((d) => (
            <div key={d.id} style={{ border: "2px solid var(--color-divider)" }}>
              ...
              <div style={{ padding: "12px 14px" }}>
                ...
                <div className="text-muted" style={{ fontSize: 12, marginTop: 4 }}>{d.address ?? "—"} · {d.market ?? "—"}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, borderTop: "1px solid var(--color-divider)", marginTop: 12, paddingTop: 10 }}>
                  <span className="text-muted" style={{ fontSize: 11, flex: 1 }}>Edited {d.updatedAt?.slice(0, 10)} · {d.ownerName}</span>
  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", borderBottom: "2px solid var(--color-divider)", paddingBottom: 12 }}>
        ...
        <strong style={{ fontSize: 15 }}>{draft?.name}</strong>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "200px minmax(0,1fr) 264px", gap: 20, alignItems: "start" }}>
          ...
          <div>
            ...
            {sections.map((s, i) => {
              ...
              return (
                <button key={s.key} onClick={() => setActive(s.key)} style={{ display: "flex", gap: 8, alignItems: "center", width: "100%", textAlign: "left", font: "inherit", cursor: "pointer", border: "none", borderLeft: `3px solid ${on ? "var(--color-accent)" : "transparent"}`, background: on ? "var(--color-surface)" : "transparent", padding: "8px 10px", fontSize: 12.5 }}>
          ...
          <div>
            ...
            <div className="text-muted" style={{ fontSize: 11, marginTop: 8 }}>8.5 × 11 in portrait · print-preview · pages stay light in both app themes.</div>
          </div>
          ...
          <div className="card elev-sm" style={{ padding: 16, gap: 12 }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 13 }}>{OM.brandKits[0].name}</div>
            <div className="text-muted" style={{ fontSize: 11.5 }}>{OM.brandKits[0].broker}</div>
            <div className="text-muted" style={{ fontSize: 11.5 }}>Archivo type · legal disclaimer included</div>
            <div style={{ borderTop: "1px solid var(--color-divider)", paddingTop: 10 }} className="text-muted">
              ...
              <span style={{ fontSize: 11.5 }}>PDF/PPTX and the Approved status unlock only after the compliance checklist is cleared.</span>
            </div>
          </div>
        </div>
      )}
...
function OmPage({ pageKey, metrics }: { pageKey: string; metrics: { metric: string; displayValue: string | null; status: string; formula: string | null }[] }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ color: "#605d5d", fontSize: 13 }}>{OM.projects[0].addr} · {OM.projects[0].market}</div>
          <div style={{ height: 2, width: 64, background: "rgba(32,30,29,.4)", margin: "12px 0" }} />
          <div style={{ fontSize: 13 }}>A corner-anchored mixed-use offering with stabilized retail and staggered residential rollover.</div>
        </div>
      </div>
    );
  }
  ...
  if (pageKey === "comps") {
    return (
      <div>
        ...
        <div style={{ borderLeft: "3px solid rgba(32,30,29,.4)", paddingLeft: 12, marginTop: 12, fontSize: 11.5, color: INK }} contentEditable suppressContentEditableWarning>Comparable sales rationale — a transparent, source-cited narrative. Comps are not a valuation.</div>
      </div>
    );
  }
      ...
      <div>
        ...
        {OM.highlights.map((h) => (
          <div key={h.n} style={{ display: "flex", gap: 12, padding: "8px 0", borderBottom: "1px solid rgba(32,30,29,.18)" }}>
            ...
            <div style={{ flex: 1, fontSize: 12 }}>{h.text}{h.flag && <div style={{ marginTop: 4 }}><span className="tag tag-outline" style={{ fontSize: 9 }}>{h.flag}</span></div>}</div>
          </div>
        ))}
      </div>
    );
  ...
  if (pageKey === "risk") {
    return (
      <div>
        ...
        {OM.risks.map((r, i) => <div key={i} style={{ display: "flex", gap: 10, padding: "6px 0", fontSize: 11.5 }}><span style={{ color: "#ae1800", fontWeight: 800 }}>{String(i + 1).padStart(2, "0")}</span><span>{r}</span></div>)}
  ...
  if (pageKey === "contact") {
    return (
      <div data-theme="dark" style={{ background: "#ec3013", color: "#f3f2f2", margin: -34, padding: 34, height: "calc(100% + 68px)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        ...
        <div style={{ borderTop: "2px solid rgba(255,255,255,.5)", paddingTop: 12, fontSize: 12 }}><strong>{OM.brandKits[0].broker}</strong><br />{OM.brandKits[0].contact}<br /><span style={{ fontSize: 9, opacity: 0.8 }}>{OM.brandKits[0].disclaimer}</span></div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {pageKey === "exec" ? (
        <div style={{ fontSize: 12, color: INK }} contentEditable suppressContentEditableWarning>Storyboard the thesis here — every sentence must be backed by an imported fact or marked [TBD — source required].</div>
      ) : (
        <div style={{ border: "2px dashed rgba(32,30,29,.4)", padding: 24, fontSize: 11.5, color: "#605d5d" }}>No sourced content yet. Attach a source — unsourced statements cannot be exported and this page will show <strong>Needs review</strong>.</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.4fr) minmax(0,1fr)", gap: 28, alignItems: "start" }}>
      <div>
        ...
        <div className="text-muted" style={{ fontSize: 12, margin: "10px 0" }}>{resolvedCount} of {findings.length} resolved · findings grouped by severity</div>
        {findings.length === 0 && <div className="text-muted" style={{ fontSize: 13 }}>No findings yet — the checklist is clear.</div>}
        {findings.map((f) => (
          <div key={f.id} style={{ display: "flex", gap: 12, padding: "12px 0", borderBottom: "1px solid var(--color-divider)", opacity: f.status === "resolved" ? 0.55 : 1 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <strong style={{ fontSize: 13 }}>{f.code.replace(/_/g, " ")}</strong>
              </div>
              <div className="text-muted" style={{ fontSize: 12.5, marginTop: 4 }}>{f.message}</div>
              {f.repairAction && <div className="text-muted" style={{ fontSize: 11.5, marginTop: 2 }}>Repair: {f.repairAction}</div>}
            </div>
      ...
      <div className="card elev-sm" style={{ padding: 18, gap: 12 }}>
        ...
        <p className="text-muted" style={{ fontSize: 12.5, margin: 0 }}>Exports unlock only after every blocking finding is resolved and you confirm. Approval is logged with a timestamp.</p>
        ...
        {blocking.length > 0 && <div className="text-muted" style={{ fontSize: 11.5 }}>{blocking.length} blocking finding(s) remain.</div>}
        {approved && (
        ...
        <div className="text-muted" style={{ fontSize: 11, borderTop: "1px solid var(--color-divider)", paddingTop: 10 }}>{OM.brandKits[0].disclaimer}</div>
      </div>
    </div>
  );
}
