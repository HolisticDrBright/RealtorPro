  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", gap: 22, maxWidth: 900 }}>
      <div className="card elev-sm" style={{ padding: 20, gap: 10 }}>
        ...
        <p style={{ fontSize: 13.5, margin: 0, maxWidth: 640 }} className="text-muted">{meta.body}</p>
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 12.5, borderTop: "1px solid var(--color-divider)", paddingTop: 10 }}>
        ...
        {meta.showErr && (
          <div role="alert" style={{ background: "var(--color-accent-100)", color: "var(--color-accent-800)", padding: "10px 14px", fontSize: 13, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div>
          ...
          {DATA.fub.permissions.map((pm) => (
            <div key={pm.scope} style={{ padding: "10px 0", borderBottom: "1px solid var(--color-divider)" }}>
              <div style={{ fontWeight: 600, fontSize: 13.5 }}>{pm.scope}</div>
              <div className="text-muted" style={{ fontSize: 12.5, marginTop: 2 }}>{pm.why}</div>
            </div>
          ))}
        </div>
        <div>
