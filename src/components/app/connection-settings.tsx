...
export function ClaudeSettings({ model, configured, verifiedAt, reload }: { model: string; configured: boolean; verifiedAt?: string | null; reload: () => void }) {
  ...
  async function save(disconnect = false) {
    setBusy(true);
    try {
      const r = await api.post("/api/integrations/connect", { kind: "claude", key: key || undefined, model: modelId, disconnect });
      if (r.ok) { setKey(""); toast(disconnect ? "Claude disconnected" : "Claude key and model verified. Connection saved."); reload(); }
  ...
  return <div className="space-y-3 rounded-lg bg-ground p-4 text-[13px]">
    ...
    <div className="flex gap-2 flex-wrap"><button className="btn btn-primary" disabled={busy || (!key && !configured)} onClick={() => save()}>{busy ? "Checking…" : "Save & test Claude"}</button>{configured && <button className="btn" disabled={busy} onClick={() => save(true)}>Disconnect Claude</button>}<a className="card-link self-center" href="https://console.anthropic.com/settings/keys" target="_blank" rel="noreferrer">Get an API key ↗</a></div>
    <p className="text-ink-3 text-xs">Saved keys are encrypted for your Windows account and never returned to the browser. The connection test checks key/model access; it does not generate paid content or verify your credit balance.{verifiedAt ? ` Last checked: ${new Date(verifiedAt).toLocaleString()}.` : ""}</p>
  </div>;
}

interface FolderListing { path: string; parent: string; isVault: boolean; folders: { name: string; path: string }[] }
export function VaultSettings({ dir, include = [], exclude = [], allowClaude, reload }: { dir: string | null; include?: string[]; exclude?: string[]; allowClaude: boolean; reload: () => void }) {
  ...
  return <div className="space-y-3 rounded-lg bg-ground p-4 text-[13px] mb-3">
    ...
    <label className="flex items-start gap-2"><input className="mt-1" type="checkbox" checked={allow} onChange={(e) => setAllow(e.target.checked)} /><span>Allow selected vault text to be sent to Anthropic for extraction and briefings. Off keeps original vault text local. Records you explicitly import into the CRM may later be included in AI features.</span></label>
    <div className="flex gap-2 flex-wrap"><button className="btn btn-primary" disabled={busy || !vaultPath} onClick={() => save()}>{busy ? "Connecting…" : "Save & connect vault"}</button>{dir && <button className="btn" disabled={busy} onClick={() => save(true)}>Disconnect vault</button>}</div>
