...
export function JarvisChat({ id }: { id?: string }) {
  ...
  useEffect(() => { try { setConsent(sessionStorage.getItem("jarvis-crm-consent") === "yes"); setVoiceConsent(sessionStorage.getItem("jarvis-voice") === "yes"); setAutoSend(sessionStorage.getItem("jarvis-auto-send") === "yes"); setReadAloud(sessionStorage.getItem("jarvis-read-aloud") === "yes"); } catch { /* session-only preferences */ } }, []);
  ...
  useEffect(() => {
    if (readAloud && turn?.status === "complete" && turn.answer && spoken.current !== turn.id) {
      ...
      speakJarvis(`${turn.drafts.length ? "Scheduling drafts need your approval. " : ""}${turn.answer}`);
    }
  }, [readAloud, turn]);
  function consentChanged(value: boolean) { setConsent(value); try { sessionStorage.setItem("jarvis-crm-consent", value ? "yes" : "no"); } catch { /* still works without persistence */ } }
  async function send(text: string) {
    ...
    try {
      const result = await api.post<{ item: JarvisTurn }>("/api/jarvis", { id: requestId, question: text.trim(), parentId: turn?.status === "complete" ? turn.id : null, consent: true, allowScheduling });
  ...
  async function decide(approve: boolean) {
    ...
    else { toast(approve ? "Approved items saved in RealtorPro" : "Drafts discarded"); current.reload(); }
  }
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_260px]">
      <div className="space-y-4 min-w-0">
        <Card title="Talk to your command center">
          <p className="text-[13px] text-ink-3">Jarvis looks up saved clients, buyers, investors, properties, off-market leads and schedules. Raw vault files, Gmail and external calendars are not included. Questions and answers are saved locally; retrieved CRM details and recent conversation context are sent to your connected Claude API when you ask.</p>
          <label className="flex items-start gap-2 text-[13px] mt-3"><input type="checkbox" className="mt-1" checked={consent} onChange={(e) => consentChanged(e.target.checked)} />Allow Jarvis to send my questions and relevant saved CRM records to Claude for this browser session.</label>
          <label className="flex items-start gap-2 text-[13px] mt-2"><input type="checkbox" className="mt-1" checked={allowScheduling} onChange={(e) => setAllowScheduling(e.target.checked)} />Allow scheduling drafts (I must approve before anything is saved).</label>
          ...
          <p className="text-[12px] text-ink-3 mt-3">Uses your existing Anthropic API billing. Up to 6 model requests per question. Calls are reminders; appointments stay in the local calendar. No dialing, emails or invitations.</p>
        </Card>
        ...
        {turn && <Card title={<span className="flex items-center gap-2">Jarvis <Badge>{turn.status}</Badge></span>}>
          ...
          {turn.drafts.length > 0 && <div className="mt-4 border-t border-line pt-4">
            <h2 className="font-semibold">{turn.reviewStatus === "applied" ? "Approved and saved in RealtorPro" : turn.reviewStatus === "pending" ? "Review before saving — drafts only" : "Drafts not applied"}</h2>
            <p className="text-[12px] text-ink-3 mt-1">All times are local to the app computer: {turn.timeZone}. Verify names, dates and times. No external invitations or calls are sent.</p>
            {turn.drafts.map((draft, i) => <div key={i} className="rounded-lg border border-line p-3 mt-3"><div className="font-medium text-sm">{label(draft.entity)}</div><dl className="text-[13px] mt-2 space-y-1">{Object.entries(draft.fields).map(([field, value]) => <div key={field} className="flex flex-wrap justify-between gap-2"><dt className="text-ink-3">{label(field.replace(/([A-Z])/g, " $1"))}</dt><dd className="break-words min-w-0">{turn.sources.find((s) => s.id === value)?.label ?? String(value ?? "—")}</dd></div>)}</dl></div>)}
