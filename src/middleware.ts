...
export function middleware(req: NextRequest) {
  ...
  // Only the OAuth callback may arrive as a cross-site top-level GET. The
  // handler separately validates a short-lived, one-use state + Lax cookie + PKCE.
  const oauthCallback = req.nextUrl.pathname === "/google/callback" && req.method === "GET" && req.headers.get("sec-fetch-mode") === "navigate" && req.headers.get("sec-fetch-dest") === "document" && !origin;
  if (origin && origin !== `http://${host}` && origin !== `https://${host}`) return deny("Cross-site requests are not allowed.");
  if (req.headers.get("sec-fetch-site") === "cross-site" && !oauthCallback) return deny("Open the app directly on localhost.");
  ...
  if (req.nextUrl.pathname.startsWith("/api/")) {
    ...
    if (bearer && req.nextUrl.pathname.startsWith("/api/google")) return deny("Google Calendar is available only through the signed-in browser and reviewed Jarvis actions.");
  ...
  if (!oauthCallback && !session && !bearer && req.headers.get("sec-fetch-mode") === "navigate" && req.headers.get("sec-fetch-dest") === "document") {
