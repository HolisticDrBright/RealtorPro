  ...
  return (
    <div className="fade-in">
      <PageHeader title="Buyers" sub={`${counts.hot} hot · ${counts.warm} warm · ${counts.nurture} nurture`}>
        ...
        {tab === "matches" && <Link className="btn" href="/buyer-match-packet">Review PDF packet</Link>}
      </PageHeader>
      ...
      {tab === "matches" && (
        <Card title="Buyer Match — active listings and opportunities vs. buyer criteria" bodyClass="!px-0">
          ...
          <table className="w-full"><thead><tr><th className="th pl-5">Score</th><th className="th">Buyer</th><th className="th">Property</th><th className="th">Why it fits</th><th className="th">Confirm</th><th className="th pr-5 text-right">Actions</th></tr></thead>
            <tbody>{matches.data?.matches.map((m) => <tr key={m.buyerId + m.candidateId} className="row-hover"><td className="td pl-5"><span className="inline-grid place-items-center w-11 h-11 rounded-lg bg-ink text-white font-semibold tnum">{m.score}</span></td><td className="td"><div className="font-medium">{m.buyerName}</div><Badge tone={m.temperature}>{m.temperature}</Badge></td><td className="td"><div className="font-medium">{m.address}</div><div className="text-[12px] text-ink-3">{[m.area, fmtMoney(m.price), m.beds && `${m.beds} bd`, m.baths && `${m.baths} ba`, m.sqft && `${m.sqft.toLocaleString()} sqft`].filter(Boolean).join(" · ")} · <Badge tone={m.kind === "listing" ? "active" : "gold"}>{label(m.kind)}</Badge></div></td><td className="td text-[12.5px]">{m.reasons.map((r) => <div key={r} className="text-ok">✓ {r}</div>)}</td><td className="td text-[12.5px] text-ink-2">{m.concerns.map((r) => <div key={r}>· {r}</div>)}</td><td className="td pr-5 text-right whitespace-nowrap"><Link className="btn btn-primary btn-sm" href={"/buyer-match-drafts?" + new URLSearchParams({ buyerId: m.buyerId, candidateId: m.candidateId, kind: m.kind })}>Match</Link><button className="btn btn-ghost btn-sm" onClick={() => quickAdd("appointments", { title: `Showing — ${m.address}`, type: "showing", contactId: data?.items.find((x) => x.id === m.buyerId)?.contactId, location: m.address })}>Schedule showing</button></td></tr>)}</tbody>
          </table>
        </Card>
      )}
