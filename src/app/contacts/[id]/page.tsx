...
export default function ContactPage() {
  ...
  const investor = useApi<{ items: { id: string; strategy: string; status: string }[] }>(`/api/investors?contactId=${id}`);
      ...
      <div className="grid gap-4 xl:grid-cols-[360px_1fr_340px]">
        <div className="space-y-4">
          ...
          <Card title={`Business with ${k.firstName}`}>
            {investor.data?.items[0] ? <div className="rounded-lg bg-ground p-3 text-[13px]"><div className="flex items-center gap-2"><span className="font-medium">Investor profile</span><Badge tone="gold">{label(investor.data.items[0].status)}</Badge><Link href={`/investors?contactId=${id}`} className="card-link">Open →</Link></div><div className="mt-1 text-ink-2">{label(investor.data.items[0].strategy)}</div></div> : investor.data && <button className="btn btn-sm mb-3" onClick={() => quickAdd("investors", { contactId: id })}>+ Investor profile</button>}
