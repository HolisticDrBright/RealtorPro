...
export function search(q: string, limit = 25): SearchHit[] {
  ...
  for (const i of db.select().from(s.investors).all()) if (has(ctx.names(i.contactId), i.strategy.replace(/_/g, " "), ...(i.targetAreas ?? []), ...(i.propertyTypes ?? []), i.notes)) hits.push({ kind: "investor", id: i.id, title: ctx.names(i.contactId) ?? "Investor", subtitle: `Investor · ${i.strategy.replace(/_/g, " ")} · ${i.status}`, href: `/investors?contactId=${i.contactId}` });
