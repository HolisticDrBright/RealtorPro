...
export function noteToBundle(fm: Record<string, unknown>, type: RecordType, title: string, body: string): Partial<ImportBundleT> {
  ...
  switch (type) {
    case "investor": return { contacts: [{ name, type: "investor", phone: str(g("phone")), email: str(g("email")), nextAction: str(g("nextAction")), nextFollowUpAt: str(g("nextFollowUp")) ?? str(g("nextFollowUpAt")),
      investor: { strategy: str(g("strategy")) as never, status: str(g("status")) as never, targetAreas: arr(g("targetAreas")) ?? arr(g("areas")), propertyTypes: arr(g("propertyTypes")), budgetMin: g("budgetMin") as never, budgetMax: g("budgetMax") as never, availableCapital: g("availableCapital") as never, targetCapRate: g("targetCapRate") as never, targetCashOnCash: g("targetCashOnCash") as never, financingType: str(g("financingType")) ?? str(g("financing")), timeline: str(g("timeline")), mustHaves: arr(g("mustHaves")), dealBreakers: arr(g("dealBreakers")), notes: notesText },
    }] };
